...
echo "🐳 Docker Build & Push Script para credit-card-processor Service"
echo "=================================================="
echo ""
...
# Verificar JAR
JAR_PATH="target/credit-card-processor-service-0.0.1-SNAPSHOT.jar"
if [ -f "$JAR_PATH" ]; then
...
CONTAINER_ID=$(docker run -d -p 8888:8081 --name credit-card-processor-service-test "$VERSIONED_TAG")
...
if [ $? -eq 0 ]; then
    ...
    if docker ps | grep -q "credit-card-processor-service-test"; then
        print_success "Contenedor ejecutándose correctamente"

        # Intentar hacer health check
        print_info "Verificando health check..."
        if docker exec credit-card-processor-service-test curl -f http://localhost:8081/actuator/health 2>/dev/null; then
        ...
        docker logs --tail 10 credit-card-processor-service-test

    else
        print_error "El contenedor se detuvo inesperadamente"
        print_info "Logs del contenedor:"
        docker logs credit-card-processor-service-test
    fi

    # Limpiar contenedor de prueba
    print_info "Limpiando contenedor de prueba..."
    docker stop credit-card-processor-service-test >/dev/null 2>&1 || true
    docker rm credit-card-processor-service-test >/dev/null 2>&1 || true
